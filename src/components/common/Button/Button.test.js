...
describe("button test", () => {
    ...
    it("should test the that the onClick function is called or not", ()=> {
        ...
        const { getByRole, debug } = render(
            <Router history={createMemoryHistory()}>
                <Button buttonCSS={buttonCSS}  buttonValue={"Login"} onClickButton={onClickButton} apiStatus={0}/>
            </Router>
        )
    ...
    it("should test that the button is disabled while it is in loading state", () => {
        ...
        const { getByRole, debug } = render(
            <Router history={createMemoryHistory()}>
                <Button buttonCSS={buttonCSS}  buttonValue={"Login"} onClickButton={onClickButton} apiStatus={100}/>
            </Router>
        )
